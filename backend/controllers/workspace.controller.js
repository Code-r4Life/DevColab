...
export const getWorkspace = asyncHandler(async (req, res) => {
  const workspace = await Workspace.findById(req.params.workspaceId)
    .populate('members.userId', 'name email avatar bio skills')
    .populate('projectCount');
...
export const deleteWorkspace = asyncHandler(async (req, res) => {
  ...
  if (workspace.ownerId.toString() !== req.user.id.toString()) {
    return fail(res, 'Access denied. Only the workspace owner can delete this workspace', 403);
  }
...
export const listMembers = asyncHandler(async (req, res) => {
  const workspace = await Workspace.findById(req.params.workspaceId)
    .populate('members.userId', 'name email avatar bio skills githubUrl');
  if (!workspace) return fail(res, 'Workspace not found', 404);
