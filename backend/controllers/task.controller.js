...
export const createTask = asyncHandler(async (req, res) => {
  ...
  if (!mongoose.isValidObjectId(projectId)) return fail(res, 'Invalid project ID', 400);
...
export const listProjectTasks = asyncHandler(async (req, res) => {
  ...
  if (!mongoose.isValidObjectId(projectId)) {
    return ok(res, req.query.grouped === 'true' ? { tasks: { todo: [], in_progress: [], in_review: [], done: [] } } : { tasks: [] });
  }

  const tasks = await populateTask(Task.find({ projectId }).sort({ status: 1, position: 1, createdAt: -1 }));
  return ok(res, req.query.grouped === 'true' ? { tasks: groupTasks(tasks) } : { tasks });
});

export const getTask = asyncHandler(async (req, res) => {
  if (!mongoose.isValidObjectId(req.params.taskId)) return fail(res, 'Invalid task ID', 400);
