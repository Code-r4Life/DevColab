...
export const createProject = asyncHandler(async (req, res) => {
  const { name, description = '', color = '#7C3AED', workspaceId, targetDate } = req.body;
  ...
  const project = await Project.create({
    ...
    createdAt: targetDate ? new Date(targetDate) : new Date()
  });
...
export const listProjects = asyncHandler(async (req, res) => {
  ...
  const counts = await Task.aggregate([
    ...
    { $group: { _id: '$projectId', tasksCount: { $sum: 1 } } }
  ]);
  ...
  return ok(res, { 
    projects: projects.map((project) => ({ 
      ...project.toJSON(), 
      tasksCount: byProject.get(project._id.toString()) || 0 
    })) 
  });
});
...
export const updateProject = asyncHandler(async (req, res) => {
  const update = {};
  ['name', 'description', 'color', 'isArchived'].forEach((key) => { 
