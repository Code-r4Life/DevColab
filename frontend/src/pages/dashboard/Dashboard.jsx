    ...
    const load = async () => {
      ...
      const taskLists = await Promise.all(
        projects.slice(0, 4).map((project) => 
          api.get(`/tasks/project/${project._id || project.id}`)
        )
      );
      ...
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            ["Team Standup", MessageSquare, "bg-success/10 text-success border-success/20"],
            ["What's Blocking?", BarChart2, "bg-info/10 text-info border-info/20"],
          ].map(([label, Icon, colorClasses]) => (
            <Link 
              key={label} 
            ...
            >
              ...
              <div>
                ...
                <p className="text-[10px] text-gray-500 uppercase font-bold tracking-widest">AI Action</p>
              </div>
            </Link>
          ))}
        </div>
            ...
            <section className="space-y-4">
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-bold flex items-center gap-2">
                  My Pending Tasks <Badge variant="primary">{pendingTasks.length}</Badge>
                </h2>
              </div>
              <div className="space-y-2">
                {pendingTasks.length === 0 && (
                  <div className="surface p-6 rounded-xl text-sm text-gray-500">No pending tasks assigned to you.</div>
                )}
                {pendingTasks.map((task) => {
                  const targetProjectId = task.projectId?._id || task.projectId;
                  return (
                    <Link 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {projects.length === 0 && (
                  <div className="surface p-6 rounded-xl text-sm text-gray-500">
                    Create your first project from onboarding or workspace settings.
                  </div>
                )}
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">{project.tasksCount || 0} tasks active</p>
                        </div>
                      </div>
                ...
                <div key={activity._id || activity.id} className="flex gap-3 relative">
                  ...
                  <div className="flex-1">
                    <p className="text-xs leading-relaxed">
                      <span className="font-bold">{activity.userId?.name || "Someone"}</span>{" "}
                      {activity.action ? activity.action.replace(".", " ") : ""}
                    </p>
                    <p className="text-[10px] text-gray-500 mt-1">{timeAgo(activity.createdAt)}</p>
                  </div>
                </div>
              ))}
            </div>
